  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      {data.slice(0, 8).map((p, pi) => {
        ...
        const oPct  = pctInt(p.rpo,  p.total);
        return (
          <div key={pi} style={{ background: "var(--bg)", borderRadius: 10,
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12,
              ...
              <div style={{ display: "flex", height: 20, width: 100,
                ...
                {oPct > 0 && (
                  <div style={{ flex: Math.max(3, oPct), background: RPO_COLOR,
                    display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {oPct > 20 && <span style={{ color: "#fff", fontSize: 9, fontWeight: 800 }}>RPO {oPct}%</span>}
                  </div>
                )}
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, padding: "12px 16px" }}>
              {p.forms.map((f, fi) => {
                ...
                const foPct  = pctInt(f.rpo,  f.total);
                ...
                return (
                  <div key={fi} style={{
                    ...
                    <div style={{ display: "flex", height: 5, borderRadius: 2,
                      ...
                      {foPct > 0 && <div style={{ flex: Math.max(2, foPct), background: RPO_COLOR }} />}
                      <div style={{ flex: Math.max(2, frPct), background: RUN_COLOR }} />
                    </div>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      <span style={{ color: PASS_COLOR, fontSize: 10, fontWeight: 700 }}>P {fpPct}%</span>
                      {foPct > 0 && <span style={{ color: RPO_COLOR, fontSize: 10, fontWeight: 700 }}>RPO {foPct}%</span>}
                      <span style={{ color: RUN_COLOR, fontSize: 10, fontWeight: 700 }}>R {frPct}%</span>
                    </div>
                  </div>
                );
              })}
...
// ── TendencyPanel (shared with GameOverview) ──────────────────────────────────
// Renders a list of TendencyCard tiles from an entries array.
export function TendencyPanel({ title, entries = [] }) {
...
export default function Overview() {
  // AppContext already polls liveRows every 3 s — no local poll needed
