...
function addStats(acc, row) {
  acc.total++;
  const pt = String(row["PLAY TYPE CALLED"] || "").trim().toLowerCase();
  if (pt === "run")  acc.run++;
  if (pt === "pass") acc.pass++;
  if (pt === "rpo")  acc.rpo++;
}
...
function ThirdDownWidget({ rows }) {
  const data = useMemo(() => {
    ...
    return buckets.map(b => {
      ...
      const run  = bucket.filter(r => String(r["PLAY TYPE CALLED"] || "").trim().toLowerCase() === "run").length;
      const pass = bucket.filter(r => String(r["PLAY TYPE CALLED"] || "").trim().toLowerCase() === "pass").length;
      const rpo  = bucket.filter(r => String(r["PLAY TYPE CALLED"] || "").trim().toLowerCase() === "rpo").length;
      return {
        label: b.label,
        total: bucket.length,
        runPct:  pctInt(run,  bucket.length),
        passPct: pctInt(pass, bucket.length),
        rpoPct:  pctInt(rpo,  bucket.length),
      };
    }).filter(Boolean);
  }, [rows]);
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse" }}>
      <thead>
        <tr>
          {["Situation", "Plays", "Run%", "Pass%", "RPO%"].map(h => (
        ...
        {data.map(row => (
          <tr key={row.label}>
            ...
            <td style={{ color: RPO_COLOR,  fontSize: 13, fontWeight: 700,
              padding: "8px 10px", borderBottom: "1px solid var(--border)" }}>{row.rpoPct > 0 ? `${row.rpoPct}%` : "—"}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
function RedZoneWidget({ rows }) {
  const data = useMemo(() => {
    ...
    function zoneStats(fpGroup) {
      ...
      const run  = zone.filter(r => String(r["PLAY TYPE CALLED"] || "").trim().toLowerCase() === "run").length;
      const pass = zone.filter(r => String(r["PLAY TYPE CALLED"] || "").trim().toLowerCase() === "pass").length;
      const rpo  = zone.filter(r => String(r["PLAY TYPE CALLED"] || "").trim().toLowerCase() === "rpo").length;
      // Top formation
      const formMap = {};
      zone.forEach(r => {
    ...
    return (
      <div style={{ flex: 1, minWidth: 180, background: "var(--bg)",
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 8, flexWrap: "wrap" }}>
          ...
          {s.rpoPct > 0 && <span style={{ color: RPO_COLOR, fontWeight: 700, fontSize: 14 }}>RPO {s.rpoPct}%</span>}
        </div>
...
function PlayByDownWidget({ rows }) {
  const data = useMemo(() => {
    ...
    return DOWN_SIMPLE.map(d => {
      ...
      const run  = dRows.filter(r => String(r["PLAY TYPE CALLED"] || "").trim().toLowerCase() === "run").length;
      const pass = dRows.filter(r => String(r["PLAY TYPE CALLED"] || "").trim().toLowerCase() === "pass").length;
      const rpo  = dRows.filter(r => String(r["PLAY TYPE CALLED"] || "").trim().toLowerCase() === "rpo").length;
      return {
        down: d,
        total: dRows.length,
